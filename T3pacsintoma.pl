...
recetar(Paciente, Medicamento) :- enfermo(Paciente, Enfermedad), alivia(Medicamento, Enfermedad).
...
tiene_sintomas_sin_diagnostico(Paciente) :- tiene_sintoma(Paciente, Sintoma), not(sintoma_de(Sintoma, _)).
